        ...
        if same == True:
            ...
            if bestError > totalError:
                ...
                if abs(bestError - 0) < 1e-6:
                    ...
                    feasSolution.to_csv(r'parallel_solution/feas_MA_1812_iter%s.csv'%(iteration), index = False)#Check
...
def phase1_revised_2(arg):
    numCores, iteration, numPlans, halfX, numDistricts, G, tolerance = arg
    return phase1_revised(numCores, iteration, numPlans, halfX, numDistricts, G, tolerance)



def phase1_revised(numCores, iteration, numPlans, halfX, numDistricts, G, tolerance):

    while iteration <= numPlans:
        ptbX = PTBX(halfX,numDistricts,G)
...
if __name__ == '__main__':
    ...
    # multiArgs = []  
    multiArgs_revised = []  
    for iteration in range(numCores): # numCores, iteration, numPlans, halfX, numDistricts, G, tolerance
        # multiArgs += [(iteration, halfX, numDistricts, G, tolerance)]  
        multiArgs_revised += [(numCores, iteration, numPlans, halfX, numDistricts, G, tolerance)]  

    results = p.map(phase1_revised_2, multiArgs_revised)
