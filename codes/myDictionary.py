...
def ROUND_NAIVE(fracX,numDistricts,G):
    district = {}
    ...
    for u in G.nodes():
        ...
        for d in range(numDistricts):
            if bestFrac < fracX[u,d]:
                bestFrac < fracX[u,d]
                bestDistrict = d
        district[bestDistrict].append(u)
...
def EQERROR_NAIVE(district,lower,upper,numDistricts,G,nx):
    totalError = 0
    for i in range(numDistricts):
        population_i = 0
        for u in district[i]:
        ...
        totalError += nx.number_connected_components(G.subgraph(district[i])) - 1
